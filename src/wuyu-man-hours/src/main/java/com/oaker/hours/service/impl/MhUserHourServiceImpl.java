...
public class MhUserHourServiceImpl extends ServiceImpl<MhUserHourMapper, MhUserHour> implements MhUserHourService {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public boolean approveUserHour(UserHourApproveDTO userHourApproveDTO) {
        EntityWrapper<MhHourDetail> wrapper = new EntityWrapper<>();
        wrapper.eq(Columns.MhHourDetail.id,userHourApproveDTO.getHourId())
                .eq(Columns.MhHourDetail.approveStatus,0);
